	...
	public:
		...
		public:
			...
			CellData(BOOL bEnabled=TRUE, BOOL bChecked=FALSE, int nRadioGroup=0, int nImage=-1, UINT uProgMaxVal=100) 
				...
				m_nImage(nImage), m_uProgresVal(0), m_uProgressMax(uProgMaxVal),m_TextColor(GetSysColor(COLOR_WINDOWTEXT)),m_TextBkColor(GetSysColor(COLOR_WINDOW))
			{
				SetEnable(bEnabled);
				SetCheck(bChecked);
			};
...
// operations
	public:
		...
		void				SetCellTextColor(int nRow, int nCol,COLORREF TextColor);
		void				SetCellTextBkColor(int nRow, int nCol,COLORREF TextBkColor);
		COLORREF            GetCellTextColor(int nRow, int nCol);
		COLORREF			GetCellTextBkColor(int nRow, int nCol);
		void				ResetCellTextandTextBkColor(int nRow, int nCol);
