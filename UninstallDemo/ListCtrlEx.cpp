...
void CListCtrlEx::SetCellTextColor(int nRow, int nCol,COLORREF TextColor)
{
	ASSERT(0<=nRow && nRow<GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	FindCellData(nRow, nCol).m_TextColor = TextColor;
}
void CListCtrlEx::SetCellTextBkColor(int nRow, int nCol,COLORREF TextBkColor)
{
	ASSERT(0<=nRow && nRow<GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	FindCellData(nRow, nCol).m_TextBkColor = TextBkColor;
}

void CListCtrlEx::ResetCellTextandTextBkColor(int nRow, int nCol)
{
	ASSERT(0<=nRow && nRow<GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	FindCellData(nRow, nCol).m_TextColor = GetSysColor(COLOR_WINDOWTEXT);
	FindCellData(nRow, nCol).m_TextBkColor = GetSysColor(COLOR_WINDOW);

}

COLORREF CListCtrlEx::GetCellTextColor(int nRow, int nCol)
{
	ASSERT(0<=nRow && nRow<GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	return FindCellData(nRow, nCol).m_TextColor;
}
COLORREF CListCtrlEx::GetCellTextBkColor(int nRow, int nCol)
{
	ASSERT(0<=nRow && nRow<GetItemCount() && nCol>=0 && nCol< GetColumnCount());
	return FindCellData(nRow, nCol).m_TextBkColor;
}
...
BOOL	CListCtrlEx::GetCheckRect(int iRow, int iCol, CRect &rect)
{
	...
	if (GetCellRect(iRow, iCol, rect))
	{
		...
		//rect.top+=1;
	//	rect.left += 7;  
		rect.left = (rect.left + rect.right)/2 - rect.Height()/2;
...
void CListCtrlEx::DrawNormal(CDC *pDC, CString &strText, CRect &rcCell, BOOL bSelected, const CellData &cellData, int uLvcFmt)
{
	...
	COLORREF crText=bSelected?GetSysColor(COLOR_HIGHLIGHTTEXT):(cellData.m_TextColor);
	COLORREF crTextBkgrnd=bSelected?GetSysColor(COLOR_HIGHLIGHT):(cellData.m_TextBkColor);
	// fill background
...
//	COLORREF crCheckMark=cellData.GetEnable()?RGB(51,255,153):RGB(192,192,192);
	COLORREF crCheckMark=cellData.GetEnable()?RGB(0,100,0):RGB(192,192,192);
	...
	rcCheck.top+=2;
	//rcCheck.left += 7;  
	rcCheck.left = (rcCell.left + rcCell.right)/2 - rcCheck.Height()/2;
	rcCheck.right = rcCheck.left + rcCheck.Height();    // width = height
	rcText.left=rcCheck.right+3;
	rcNull.right = rcCheck.left -4;
	...
	pDC->FillSolidRect(&rcNull,crTextBkgrnd);
